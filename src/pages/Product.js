        ...
        try {
            ...
            await axios.post(url, product, config.headers()).then(res => {
                if (res.data.message === 'success') {
                    ...
                    fetchData();
                    // clearForm();
                    // document.getElementById('btnModalclose').click(); //ใช้ได้กรณีที่มี modal อันเดียวในหน้านั้นๆ
                    handleClose();
                }
            })
                ...
                <form onSubmit={handleSave}>
                    <div className='row'>
                        <div className='mt-3 col-3'>
                            <label>barcode</label>
                            <input value={product.barcode} onChange={e => setProduct({ ...product, barcode: e.target.value })} className='form-control'></input>
                        </div>
                        <div className='mt-3 col-9'>
                            <label>ชื่อสินค้า</label>
                            <input value={product.name} onChange={e => setProduct({ ...product, name: e.target.value })} className='form-control'></input>
                        </div>
                    </div>
                    <div className='row'>
                        <div className='mt-3 col-2'>
                            <label>ราคาทุน</label>
                            <input value={product.cost} onChange={e => setProduct({ ...product, cost: e.target.value })} className='form-control'></input>
                        </div>
                        <div className='mt-3 col-2'>
                            <label>ราคาจำหน่าย</label>
                            <input value={product.price} onChange={e => setProduct({ ...product, price: e.target.value })} className='form-control'></input>
                        </div>
                        <div className='mt-3 col-8'>
                            <label>รายละเอียด</label>
                            <input value={product.detail} onChange={e => setProduct({ ...product, detail: e.target.value })} className='form-control'></input>
                        </div>
                    </div>
                                ...
                                <div className='card-body text-center'>
                                    {item.isMain ?
                                        ...
                                        <button onClick={e => handleChooseMainImage(item)}
                                            className='btn btn-default mr-2'>ภาพหลัก</button>
                                    }
