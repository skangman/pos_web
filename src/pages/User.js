...
function User() {
    ...
    const changPassword = (item) => {
        setPassword(item);
        comparePassword();
    }

    const changPasswordConfirm = (item) => {
        setPasswordComfirm(item);
        comparePassword();
    }

    const comparePassword = () => {
        if (password.length > 0 && passwordComfirm.length > 0) {
            if (password != passwordComfirm) {
                Swal.fire({
                    title: 'ตรวจสอบการกรอกรหัสผ่าน',
                    text: 'โปรดกรอกระหัสผ่าน ให้ตรงกัน',
                    icon: 'error'
                });
            } else {
                setUser({
            ...
            <Modal id="modalUser" title="ผู้ใช้งานระบบ" modalSize="modal-lg">
                ...
                <div className="mt-3">
                    ...
                    <input value={password}
                        ...
                        onBlur={e => changPassword(e.target.value)}
                        type="password" className="form-control" />
                </div>
                <div className="mt-3">
                    <label>ยืนยัน Password</label>
                    <input value={passwordComfirm} onChange={e => setPasswordComfirm(e.target.value)}
                        onBlur={e => changPasswordConfirm(e.target.value)}
                        type="password" className="form-control" />
                </div>
                <div className="mt-3">
                    <label>ระดับ</label>
